...
const dragAndDropListLocalStorage = () => {
    // 這個state是用來記錄每個item的內容
    const [items, setItems] = useState<Item[]>(initialState);

    // 新增 useEffect 來讀取 localStorage
    useEffect(() => {
        const items = JSON.parse(localStorage.getItem("option"));
        if (items) {
            setItems(items);
        }
    }, []);
    // 新增 useEffect 來寫入 localStorage
    useEffect(() => {
    ...
    // 檢查localStorage目前的狀態
    useEffect(() => {
        console.log("localStorage", localStorage.getItem("option"));
    }, [items]);
    ...
    return (
        <div className={`${index.App}`}>
            <div className={`${style_homework02.font_size} ${style_homework02.page_styles}`}>
                <ClickButton onClick={handleAddClick} text="Add" />
                <ClickButton onClick={handleRemoveClick} text="Remove" />

                {items.map((item, index) => (
                    // 增加 draggable 屬性，讓元素可以被拖動
                    <div key={uuid()} className="border padding flex justify-center" draggable="true"
                        // 綁定事件
...
export default dragAndDropListLocalStorage;